...
from app.rest.schemas.trip import TripCreate, TripUpdate, Trip
...
async def read_trip(id: str, db= Depends(get_db), current_user: str = Depends(JWTBearer())):
...
async def delete_existing_trip(id: str, current_user: str = Depends(JWTBearer()), db= Depends(get_db)):
    trip = await get_trip_by_id(db, id)
    if trip["user_id"] != current_user:
        raise HTTPException(status_code=401, detail="Unauthorized")
