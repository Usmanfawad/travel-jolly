...
async def read_activity(id: str, db= Depends(get_db)):
    return await get_activity_by_id(db, id)

@router.post("/activities", response_model=Activity)
async def create_new_activity(activity: ActivityCreate, current_user: str = Depends(auth_deps), db= Depends(get_db)):
    return await create_activity(db, activity)

@router.put("/activities/{id}", response_model=Activity)
async def update_existing_activity(id: str, activity_update: ActivityUpdate, current_user: str = Depends(auth_deps), db= Depends(get_db)):
...
async def delete_existing_activity(id: str, current_user: str = Depends(auth_deps), db= Depends(get_db)):
